// Selected SWS List
// Refined SWS ID:	CANNM102
// Refined SWS ID:	CANNM103
// Refined SWS ID:	CANNM106
// Refined SWS ID:	CANNM107
...
public class TC_CANNM_0088 extends TC_CANNM {
	public static double execute(String conf)
			throws ParserConfigurationException, SAXException, IOException,
			XPathExpressionException, SecurityException,
		...
		// To Do: Write your test case code.
		// Step 1
		...
		for (int i = 0; i < 1; i++) {
			...
			v_NetworkHandle = i;
			// v_ExpCanIfTransmit =
			// func.mCanNmDefaultCanIfTransmitSettings(v_ChannelIndex);

			System.out.println("\nChannel:\t" + v_NetworkHandle);
			// Step 3: CanNmRepeatMessageTime > 0 ? proceed , fail
			// if (cfg.chConf.get(0).CanNmRepeatMessageTime > 0) {
			// Step 4&5
			v_ChannelIndex = func.mCanNmNetworkRequest("VP05", v_NetworkHandle,
					NM_E_OK);
			...
			// Step 7 & 8
			func.mCanNmGetState("VP08", v_NetworkHandle,
					Nm_StateType.NM_STATE_REPEAT_MESSAGE,
					Nm_ModeType.NM_MODE_NETWORK, Nm_ReturnType.NM_E_OK);

			// Step 9 & 10
			v_ChannelIndex = func.mCanNmNetworkRelease("VP10", v_NetworkHandle,
					NM_E_OK);
			...
			// Step 12 & 13
			func.mCanNmGetState("VP13", v_NetworkHandle,
			...
			// Step 16 & 17
			func.mCanNmGetState("VP17", v_NetworkHandle,
					Nm_StateType.NM_STATE_PREPARE_BUS_SLEEP,
					Nm_ModeType.NM_MODE_PREPARE_BUS_SLEEP,
					Nm_ReturnType.NM_E_OK);

			// Step 18 & 19
			v_ChannelIndex = func.mCanNmNetworkRequest("VP19", v_NetworkHandle,
					NM_E_OK);
			...
			// Step 21 & 22
			func.mCanNmGetState("VP22", v_NetworkHandle,
					Nm_StateType.NM_STATE_REPEAT_MESSAGE,
					Nm_ModeType.NM_MODE_NETWORK, Nm_ReturnType.NM_E_OK);
